...
<cfcomponent hint="Tokenizer v.1.2">
    ...
    <cffunction name="writeTokenToPage" access="public" returntype="string" hint="creates the hidden form input for doing tokenization">
    	<cfargument name="tokenName" required="true" type="string">
		<cfargument name="inputName" type="string" required="false" default="xsrf_token">

        <cfscript>
			return '<input type="hidden" name="#arguments.inputName#" id="#arguments.inputName#" value="' & variables.tokenStore[arguments.tokenName].token & '" />';
		</cfscript>
    </cffunction>
